...
describe("RyanOSApp mode switching", () => {
  ...
  it("syncs project slugs into browser history", async () => {
    const user = userEvent.setup();
    window.history.replaceState(null, "", "/projects/cicd-quality-gates");

    render(
      <RyanOSApp initialRoute={resolveWorkspaceRouteFromPathname(window.location.pathname)} />
    );

    expect(await screen.findByRole("heading", { name: "CI/CD Quality Gates" })).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /Performance & Load Testing/ }));

    expect(window.location.pathname).toBe("/projects/performance-load-testing");
  });
});
