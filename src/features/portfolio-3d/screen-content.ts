...
import { performanceScenarios, performanceThresholds } from '@/features/performance-lab/domain/performance';
...
export const portfolio3dScreenPreviews = [
  ...
  },
  {
    ...
    eyebrow: 'full stack',
    title: 'Build path',
    lines: compactStrings([frontendNode?.label, apiNode?.label, backendNode?.label, dataNode?.label]),
    accent: '#7fc8ff'
  },
  {
    sectionId: 'backend',
...
> = {
  overview: {
    ...
    tags: uniqueStrings([
      profile.role,
      ...profile.focusAreas,
      ...compactStrings([buildCapability?.title, qualityCapability?.title, deliveryCapability?.title])
  ...
  profile: {
    ...
    eyebrow: 'profile',
    title: profile.name,
    summary: profile.tagline || profile.summary,
    blocks: [
      {
        heading: 'Current positioning',
        items: compactStrings([profile.role, profile.availability, profile.yearsOfExperience])
      },
      {
        heading: 'Focus areas',
        items: profile.focusAreas.slice(0, maxPanelItems)
    ...
    tags: uniqueStrings([profile.role, ...profile.focusAreas]).slice(0, maxPanelTags),
    links: primaryContactLinks,
    emptyLabel: 'Profile data is not configured yet.'
  },
  experience: {
    sectionId: 'experience',
    eyebrow: 'career shape',
    title: 'Career arc, not resume detail',
    summary: 'Public view keeps the story short. Company names, dates, and full responsibilities stay in the CV.',
    blocks: publicExperience.slice(0, 3).map((item) => ({
      heading: item.role,
      body: item.summary,
      items: item.technologies.slice(0, 4)
    })),
    tags: uniqueStrings(publicExperience.flatMap((item) => item.technologies)).slice(0, maxPanelTags),
    links: contactLinks.filter((link) => link.label === 'CV'),
    emptyLabel: 'Public experience summary is not configured yet.'
  },
  projects: {
    sectionId: 'projects',
    eyebrow: 'projects',
    title: 'Project systems',
    summary: 'Public-safe case studies show product build, automation, performance, and delivery thinking.',
  ...
  fullstack: {
    ...
    summary: buildCapability?.description ?? 'Frontend, API, backend, and data work as one application system.',
    blocks: [
      {
        heading: 'Build flow',
    ...
    tags: uniqueStrings([...(buildCapability?.technologies ?? []), ...(dataCapability?.technologies ?? [])]).slice(
      0,
      maxPanelTags
    ),
    links: [],
  ...
  backend: {
    ...
    summary: backendNode?.description ?? 'Backend services connect route contracts, validation, data, and integration work.',
    blocks: [
      {
        heading: 'API surface',
        items: apiEndpoints.slice(0, maxPanelItems).map((endpoint) => `${endpoint.method} ${endpoint.path}`)
      },
      {
    ...
    tags: uniqueStrings([...(backendNode?.technologies ?? []), ...(apiNode?.technologies ?? [])]).slice(
      0,
      maxPanelTags
    ),
    links: [],
  ...
  architecture: {
    ...
    summary: 'Public-safe maps connect build, quality, data, and delivery decisions without exposing private systems.',
    blocks: [
      {
      ...
      {
        heading: 'Core nodes',
        items: architectureNodes.slice(0, maxPanelItems).map((node) => `${node.label}: ${node.layer}`)
      }
    ],
  ...
  automation: {
    ...
    summary: qualityCapability?.description ?? 'Automation makes web, mobile, API, and release signals repeatable.',
    blocks: [
      ...skillGroups
  ...
  performance: {
    ...
    summary: 'Scenario data shows how latency, checks, and threshold posture can inform release decisions.',
    blocks: [
      {
        heading: 'Scenarios',
    ...
    tags: uniqueStrings([...(qualityCapability?.technologies ?? []), 'Performance Testing']).slice(0, maxPanelTags),
    links: [],
  ...
  pipeline: {
    ...
    tags: uniqueStrings([...(deliveryCapability?.technologies ?? []), ...(cicdNode?.technologies ?? [])]).slice(
      0,
      maxPanelTags
    ),
    links: [],
  ...
  terminal: {
    ...
    summary: 'Terminal commands expose the same public portfolio data through a concise engineering interface.',
    blocks: skillGroups.slice(0, maxPanelItems).map((group) => ({
      heading: group.title,
      items: group.skills.slice(0, 3).map((skill) => skill.name)
    })),
    tags: uniqueStrings(skillGroups.flatMap((group) => group.skills.map((skill) => skill.name))).slice(0, maxPanelTags),
    links: [],
...
function isConfiguredSafeContactLink(link: ContactLink): boolean {
  return link.href.trim().length > 0 && link.value.trim().length > 0 && isPortfolio3dSafeHref(link.href);
}
